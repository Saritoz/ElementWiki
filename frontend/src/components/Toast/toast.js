...
const Toast = ({ active, type = "info", message = "Empty message" }) => {
  const [showMessage, setShowMessage] = useState(active);
  console.log(showMessage);

  useEffect(() => {
    const countdown = setTimeout(() => setShowMessage(false), 4900);
    return () => clearTimeout(countdown);
  }, []);

  return (
    <div className="fixed top-[80px] right-8 z-50">
      <div
